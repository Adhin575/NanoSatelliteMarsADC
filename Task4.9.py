...
# ════════════════════════════════════════════════════════════════════
#  NADIR-POINTING REFERENCE FRAME  (Task 4)
# ════════════════════════════════════════════════════════════════════
def _R1(a):
...
def dcm_HN(t):
    ...
    return _R3(th) @ _R1(I_LMO) @ _R3(OMEGA_LMO)

def dcm_RnN(t):
    """[RnN] = diag(-1,1,-1) @ [HN](t)
    Rows: r̂₁=−î_r (nadir), r̂₂=î_θ (track), r̂₃=−î_h (RH)
    """
    return np.diag([-1., 1., -1.]) @ dcm_HN(t)

def omega_RnN(t):
    """NωRn/N = θ̇ · î_h = θ̇ · row2 of [HN]  (N-frame)"""
...
def simulate(t_end, dt=1.0):
    ...
    # last step control
    RN = dcm_RnN(t_h[-1]); oRN = omega_RnN(t_h[-1])
    sBR, oBR = attitude_error(X[:3], X[3:], RN, oRN)
...
def validate():
    print("="*64)
    print("ASEN 5010  —  Task 9: Nadir-Pointing PD Control")
    print("="*64)
    print(f"\n  K = {K_GAIN:.10f} N·m   (= 1/720)")
    print(f"  P = {P_GAIN:.10f} N·m·s (= 1/6)")
    print(f"  Reference: [RnN](t) — nadir frame (time-varying)")
    ...
    # ── Tracking error at check times ────────────────────────
    print(f"\n{'─'*64}")
    print("Attitude tracking error |σ_B/R| (convergence check):")
    print(f"  {'t(s)':>5}  {'|σ_B/R|':>12}  {'|BωB/R| (rad/s)':>18}")
    print(f"  {'─'*5}  {'─'*12}  {'─'*18}")
    for tc in check:
        s = X_h[tc,:3]; w = X_h[tc,3:]
        sBR, oBR = attitude_error(s, w, dcm_RnN(tc), omega_RnN(tc))
    ...
    fig.suptitle(
        "Task 9: Nadir-Pointing PD Control\n"
    ...
    ax = axes[0]
    for j in range(3):
        ax.plot(t_h, X_h[:, j], color=cols[j],
                lw=1.8, label=lbl_s[j])
    ...
    ax = axes[2]
    for j in range(3):
        ax.plot(t_h, u_h[:, j], color=cols[j],
                lw=1.8, label=lbl_u[j])
    ax.axhline(0, color='k', lw=0.8, ls='--')
